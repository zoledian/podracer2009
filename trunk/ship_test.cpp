...
void Ship::gravity(GLdouble yDistance, GLdouble angle)
{
  ...
  if ((yDistance >= 0.6) && (!_jumping))
    {
      _location[1] = _location[1] - 0.10;
      _falling = true;
    }

  cout << "Location1 " << _location[1];

  _location[1] = _location[1] + (currentSpeed * tan(angle*(M_PI/180)));
  _location[2] = _location[2] - currentSpeed; //- (currentSpeed*cos(angle*(M_PI/180)));

  // Compensate for block angle
  if (angle != _angle)
    _angle = angle;

  cout << " Location2 " << _location[1];
...
{
  _falling = false;
  GLdouble compensateDistance = 0.0;

  // Make doubles behave
  if (yDistance < 0.05 && yDistance > -0.05)
    yDistance = 0.0;
  else if ( (yDistance < 0.520) || (yDistance > 0.480))
    compensateDistance = (0.500 - yDistance);

  
    //else if (yDistance < 0.501 & yDistance > 0.499)
    //yDistance = 0.500;
  ...
  //      && yDistance > 0.05)
  //    {
  //      _location[1] += (0.5 - yDistance);
  //      //_location[1] = _location[1] + 0.06;
  //    }
  //  else if ((yDistance > 0.5) && (!_jumping))
  //    {
  //      _location[1] = _location[1] - 0.10;
  //      _falling = true;
  //    }

  if ((yDistance == 0.0) && (!_jumping))
    {
      _location[1] = _location[1] - 0.10;
      _falling = true;
    }
  else if (!_jumping)
    _location[1] = _location[1] + compensateDistance;
  ...
  cout << "Ship Y: " << _location[1] << " yDistance: " << yDistance << " compensateDistance: " << compensateDistance << endl << endl;;
...
void Ship::printHighscore()
{
  ...
  ss << "yDistance: " << (GLdouble) _yDistance << " _location1: " << (GLdouble) _location[1];
